...
def review_json(db, r, student=False):
    ...
    results = [{'criterionId': c['criterion_id'], 'aiSuggestedScore':None if student else c.get('ai_suggested_score',c.get('suggested_score')), 'evidenceAnchors':[] if student else [{'artifactId':e.get('artifact_id',''), 'path':e.get('artifact',''), 'start':e.get('anchor',{}).get('start',''), 'end':e.get('anchor',{}).get('end',e.get('anchor',{}).get('start','')), 'quote':e.get('quote','')} for e in c.get('evidence',[])], 'suggestedScore': None if student else c.get('suggested_score'), 'finalScore': c.get('final_score'), 'confidence': 0 if student else c.get('confidence', 0), 'abstained': False if student else c.get('abstained', False), 'reason': c.get('note', '') if student else c.get('reason', c.get('reasoning', '')), 'confirmed': c.get('confirmed', False), 'note': c.get('note', '')} for c in r.criterion_results]
    annotations = []
    for a in r.annotations:
        ...
        annotations.append({'id': a['id'], 'criterionId': a.get('criterion_id'), 'category': a.get('category', 'comment'), 'source': a.get('source', 'ai'), 'status': a.get('status', 'pending'), 'message': a.get('message', ''), 'advice': a.get('advice', ''), 'visibleToStudent': a.get('visible_to_student', True), 'anchor': {'artifactId': anchor.get('artifact_id', ''), 'path': anchor.get('path', ''), 'start': str(anchor.get('start', '')), 'end': str(anchor.get('end', '')), 'quote': anchor.get('quote', '')}})
